...
# Step 1: Join the flights dataset with the planes dataset
flights_with_size <- flights %>%
  inner_join(planes, by = "tailnum") # Join on tailnum to bring in the 'size' column

# Step 2: Count planes by size and airport
...
```{r}
ggplot(size_by_airport_counts, aes(x = size, y = count, fill = size)) +
  geom_bar(stat = "identity") +
  facet_wrap(~origin) + # Facet by airport
  labs(
    title = "Counts of Small, Medium, and Large Planes by Airport",
